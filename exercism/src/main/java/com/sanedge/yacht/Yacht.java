...
public class Yacht {
  ...
  int score() {
    List<Function<List<Integer>, Integer>> functions = List.of(
        ...
        dice -> Set.of(2, 3)
            .equals(dice.stream().distinct().map(d -> Collections.frequency(dice, d)).collect(Collectors.toSet()))
                ? dice.stream().reduce(Integer::sum).orElse(0)
                : 0,
